...
class SyntAnomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print(f"After color conversion: {img.shape}")
        #print(f"Resizing to: width={self.image_width}, height={self.image_height}")
        img = cv2.resize(img, dsize=(self.image_width, self.image_height))
        #print(f"After resize: {img.shape}")
        img = aug(image=img)
        #print(f"After augmentation: {img.shape}")
        img = img.astype(np.float32)
        img = img / 255.0
        img = img.transpose((2, 0, 1))
        #print(f"Final shape after transpose: {img.shape}")
        img = (img - 0.5) * 2
